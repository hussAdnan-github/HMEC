...
import { 
  ...
  createEducationalLevelServerAction, 
  updateEducationalLevelServerAction, 
  deleteEducationalLevelServerAction 
} from '@/actions/hrActions';
import type { ApiEducationalLevel } from '@/types/api';
import { DeleteConfirmModal } from '@/components/dashboard/Modals';
import { ToastNotification, ToastMessage } from '@/components/ui/ToastNotification';

export default function EducationalLevelsPage() {
  const [data, setData] = useState<ApiEducationalLevel[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<ApiEducationalLevel | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getEducationalLevelsServerAction();
      if (res && res.success) {
  ...
  const handleOpenEdit = (item: ApiEducationalLevel) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await deleteEducationalLevelServerAction(deleteTarget.id);
      if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const res = await updateEducationalLevelServerAction(editingId, formData);
        ...
        const res = await createEducationalLevelServerAction(formData);
        if (res.success && res.data) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      ...
      <div className="flex items-center text-sm text-muted-foreground gap-2">
        ...
        <span className="text-foreground font-medium">المستويات التعليمية</span>
      </div>

      {/* Header */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">المستويات التعليمية</h1>
          <p className="text-muted-foreground text-sm mt-1">إدارة المستويات والدرجات العلمية في النظام</p>
        </div>
        <button 
      ...
      {/* <div className="bg-card border border-border rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="relative w-full sm:w-72">
          ...
          <input 
            type="text" 
            placeholder="بحث عن مستوى تعليمي..." 
            value={search}
          ...
          <table className="w-full text-right text-sm">
            <thead className="bg-muted/50 text-muted-foreground">
              <tr>
                ...
                <th className="px-6 py-4 font-bold">المستوى التعليمي</th>
                ...
                !loading && (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-muted-foreground font-medium">
                      لا توجد مستويات تعليمية مضافة حالياً أو مطابقة للبحث.
                    </td>
                  </tr>
                )
              )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-card w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h2 className="text-lg font-bold">{editingId ? 'تعديل المستوى التعليمي' : 'إضافة مستوى تعليمي جديد'}</h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-bold text-foreground">اسم المستوى <span className="text-red-500">*</span></label>
                <input 
                  type="text"
                  required
                  value={formData.name}
                  ...
                  placeholder="أدخل اسم المستوى (مثال: بكالوريوس, ماجستير)..."
                />
              </div>
